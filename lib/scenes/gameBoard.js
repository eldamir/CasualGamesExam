...
GameBoardScene.prototype.onTick = function () {
    ...
    while (part.next) {
        if (hitTest(this.player.body, part)
            && player1Length > this.player.initialBodyPartCount  // Must eat 1 mice before you can die
        ...
        if (this.rules.multiPlayer) {
            if (hitTest(this.player2.body, part)
                && this.player2.length() > this.player2.initialBodyPartCount  // Must eat 1 mice before you can die
