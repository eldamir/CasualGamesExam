...
function Mouse(options) {
    ...
    this.geometry = new createjs.Container();

    this.geometry = new createjs.Sprite(this.mouseSheet, this.currentAnimation);

    // Placing the mice randomly
    var extraSpace = this.game.stage.canvas.width - this.game.currentScene.gameBoard.width;
    var randPosX = getRandomInt(0 + extraSpace,  this.game.currentScene.gameBoard.width + extraSpace);
    var randPosY = getRandomInt(0, this.game.currentScene.gameBoard.height);
    // TODO: make sure the mice wont generate in the walls.
